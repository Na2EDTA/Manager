<template>
  <div>
    <div style="padding: 10px 0;">
                <el-input style="width: 200px; margin-left: 5px" placeholder="请输入姓名" suffix-icon="el-icon-search"></el-input>
                <el-input style="width: 200px; margin-left: 5px" placeholder="请输入id" suffix-icon="el-icon-search" v-model="id"></el-input>
                <el-input style="width: 200px; margin-left: 5px" placeholder="请输入手机号" suffix-icon="el-icon-search"></el-input>
...
export default {
  ...
  created () {
    fetch("http://localhost:8082/jat/api/userpage?pageNum="+this.pageNum+"&pageSize="+this.pageSize+"&id="+this.id).then(res => res.json()).then(res => {
            console.log(res)
